...
@Configurable
@Autonomous(name = "BLUE: Front - Open Gate - 6")
public class Auto_BLUE_Front_6_OpenGate extends OpMode {
    ...
    private Supplier<PathChain> closeShotPoint, firstCollectionChain_0, firstCollectionChain_1, secondCollectionChain_0, secondCollectionChain_1, endingChain, customShotPathChain, openGateChain;
...
//        sorter.shift(0);
        ...
        closeShotPoint = () -> follower.pathBuilder() //Lazy Curve Generation
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(50, 98))))
        ...
        secondCollectionChain_0 = () -> follower.pathBuilder() //Lazy Curve Generation
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(48, 63))))
                .setHeadingInterpolation(HeadingInterpolator.constant(Math.toRadians(180)))
                .build();
        ...
        endingChain = () -> follower.pathBuilder() //Lazy Curve Generation
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(58, 112))))
...
//                    Log.d("Shooter14", "Sorter Empty End Shooting and Reset Timers");
    ...
    public void loop() {
        ...
        switch(pathState){
            ...
            case 4:
                if(!follower.isBusy() && (timer.milliseconds() - pathChainTimer) >= 3000){
                    follower.followPath(openGateChain.get(), true);
                    pathState = 5;
                    pathChainTimer = timer.milliseconds();
                    delayTimer[5] = timer.milliseconds();
                }
            ...
            case 5:
                if ((!follower.isBusy() && timerExpired(5,3000)) && (timer.milliseconds() - pathChainTimer) >= 4000) {
                    shooter.setVelocity("Low");
                    follower.followPath(closeShotPoint.get(), true);
                    pathState = 6;
                    pathChainTimer = timer.milliseconds();
