...
@Configurable
@Autonomous(name = "RED: Front - Open Gate - 6")
public class Auto_RED_Front_6_OpenGate extends OpMode {
    ...
    private Supplier<PathChain> closeShotPoint, firstCollectionChain_0, firstCollectionChain_1, secondCollectionChain_0, secondCollectionChain_1, endingChain, customShotPathChain, closeShotBezierCurve, gateOpenChain;
...
//        sorter.shift(0);
        ...
        gateOpenChain = () -> follower.pathBuilder() //Lazy Curve Generation
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(118, 80))))
                .setHeadingInterpolation(HeadingInterpolator.constant(Math.toRadians(45)))
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(129, 75))))
...
//                    pathState++; //Move on to next autonomous step
    ...
    public void loop() {
        ...
        switch(pathState){
            ...
            case 4:
                if(!follower.isBusy() && (timer.milliseconds() - pathChainTimer) >= 3000){
                    follower.followPath(gateOpenChain.get(), true);
                    pathState = 5;
                    pathChainTimer = timer.milliseconds();
                    delayTimer[5] = timer.milliseconds();
                }
