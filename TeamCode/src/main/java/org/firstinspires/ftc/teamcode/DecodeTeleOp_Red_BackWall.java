...
private Follower follower;
    ...
    public void init() {
        ...
        farShotPathChain = () -> follower.pathBuilder() //Lazy Curve Generation
                ...
                .setHeadingInterpolation(HeadingInterpolator.linearFromPoint(follower::getHeading, Math.toRadians(65), .8))
                .build();

        closeShotPathChain = () -> follower.pathBuilder() //Lazy Curve Generation
                .addPath(new Path(new BezierLine(follower::getPose, new Pose(98, 98))))
        ...
        if(gamepad1.guide || gamepad1.guideWasReleased()){
            ...
            if(!sorter.isEmpty()){
                ...
                customShotPathChain = () -> follower.pathBuilder() //Lazy Curve Generation
                        .addPath(new Path(new BezierLine(follower::getPose, new Pose(follower.getPose().getX() + 1, follower.getPose().getY() + 1, customParameters[1])))) // No change in x,y.
                        .setConstantHeadingInterpolation(Math.toRadians(customParameters[1]))
                        //.setHeadingInterpolation(HeadingInterpolator.linearFromPoint(follower::getHeading, Math.toRadians(customParameters[1]), .8)) // Change heading
                        .build();
